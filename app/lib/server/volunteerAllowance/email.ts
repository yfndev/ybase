...
export async function sendApprovalEmail(id: string): Promise<void> {
    ...
    html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr><td style="${CELL_STYLE}"><strong>Ehrenamtliche/r:</strong></td><td style="${CELL_STYLE}">${escapeHtml(data.volunteerName)}</td></tr>
            <tr><td style="${CELL_STYLE}"><strong>Projekt:</strong></td><td style="${CELL_STYLE}">${escapeHtml(data.project.name)}</td></tr>
            <tr><td style="${CELL_STYLE}"><strong>Tätigkeit:</strong></td><td style="${CELL_STYLE}">${escapeHtml(data.activityDescription)}</td></tr>
...
export async function sendRejectionEmail(id: string): Promise<void> {
  const data = await getWithDetails(id);
  if (!data) return;
  if (!data.organization.accountingEmail) return;
