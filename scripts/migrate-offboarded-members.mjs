...
try {
  ...
  const filter = { memberStatus: "offboarded" };
  const currentCount = await members.countDocuments(filter);

  console.info(
    `${apply ? "Apply" : "Dry run"}: ${currentCount} offboarded member(s) in "${databaseName}".`,
  );

  if (!apply) {
    console.info(
      `Re-run with --apply --expected-count=${currentCount} after the new member lifecycle is deployed.`,
    );
  } else {
    ...
    if (currentCount !== expectedCount) {
      throw new Error(
        `Expected ${expectedCount} offboarded members, found ${currentCount}.`,
      );
    }
    ...
    const result = await members.updateMany(filter, [
      {
        $set: {
          ...
          archivedAt: {
            $ifNull: ["$archivedAt", { $ifNull: ["$offboardedAt", now] }],
          },
        },
      },
    ]);
    ...
    console.info(
      `Matched ${result.matchedCount}, modified ${result.modifiedCount}, remaining legacy ${remainingCount}, archived total ${archivedCount}.`,
    );
